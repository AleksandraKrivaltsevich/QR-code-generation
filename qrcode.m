...
fprintf('Эта программа генерирует QR-коды для введенного вами текста.\n');

%% Получение текста из глобальной переменной
%global txt;
%count = length(txt);
txt = input('Пожалуйста, введите текст для кодирования: ' , 's');
count = length(txt);
%% Определение уровня коррекции ошибок
ecc = input('Выберете уровень коррекции ошибок из представленных "Q", "L", "H" или "M": ' , 's');
ver = 1;
coll = 1;
%global ecc;
%ecc = handles.ecc_level;
